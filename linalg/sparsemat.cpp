     ...
     isSorted(false)
{
   ...
   NodesMem = nullptr;
#endif
   mfem_error("\n\nDATA COPIED - USE STD::VECTOR: SparseMatrix\n\n");
}

SparseMatrix::SparseMatrix(int *i, int *j, double *_data, int m, int n,
     ...
     isSorted(issorted)
{
   ...
   if ( A == nullptr )
   {
	 ...
	 data = std::vector<double>(nnz, 0);
   }
   else
   {

	 indices = std::vector<int>(j, j + i[m]);
	 data = std::vector<double>(_data, _data + i[m]);
   }
...
SparseMatrix::SparseMatrix(int nrows, int ncols, int rowsize)
   ...
   , indptr(nrows + 1)
   , indices(nrows * rowsize)
   , data(nrows * rowsize, 0)
   , Rows(nullptr)
   , ColPtrJ(nullptr)
   , ColPtrNode(nullptr)
   , ownGraph(true)
...
SparseMatrix::SparseMatrix(const SparseMatrix &mat, bool copy_graph)
   : AbstractSparseMatrix(mat.Height(), mat.Width()),
	 indptr(mat.indptr), indices(mat.indices),
	 data(mat.data)
{
   I = indptr.data();
   J = indices.data();
   ...
   if (mat.Finalized())
   {
      ...
      {
		  mfem_error("Don't use this: SparseMatrix::SparseMatrix shallow copy");
         I = mat.I;
         J = mat.J;
         ownGraph = false;
      }
...
{
	mfem_error("Don't use this: SparseMatrix::MakeRef");
	/*
...
int SparseMatrix::RowSize(const int i) const
{
   ...
   if (indptr.size() > 0)
   {
	  return indptr[gi + 1] - indptr[gi];
   }

   int s = 0;
   RowNode *row = Rows[gi];
   for ( ; row != nullptr; row = row->Prev)
      if (row->Value != 0.0)
      {
         s++;
      }
...
void SparseMatrix::SetWidth(int newWidth)
{
   ...
   else if (newWidth > width)
   {
      ...
      {
         delete [] ColPtrNode;
         ColPtrNode = static_cast<RowNode **>(nullptr);
      }
      else
      {
         delete [] ColPtrJ;
         ColPtrJ = static_cast<int *>(nullptr);
      }
      width = newWidth;
   }
   else
...
void SparseMatrix::SortColumnIndices()
{
   ...
   std::transform(begin(perm), end(perm),
			      begin(sortedIndices),
				  [&](int i) {
				  return indices[i];
				  });
...
void SparseMatrix::MoveDiagonalFirst()
{
   ...
   for (int i = 0; i < height; ++i)
   {
	   ...
	   MFEM_VERIFY(index >= 0, "diagonal entry not found in row = " << i);

	   for (int j = index; j > indptr[i]; ++j)
	   {
...
const double &SparseMatrix::operator()(int i, int j) const
{
   ...
   {
      for (RowNode *node_p = Rows[i]; node_p != nullptr; node_p = node_p->Prev)
      {
         if (node_p->Column == j)
         {
            return node_p->Value;
...
double SparseMatrix::InnerProduct(const Vector &x, const Vector &y) const
{
   ...
   for (int i = 0; i < height; i++)
   {
      ...
      if (A)
         ...
         for (RowNode *np = Rows[i]; np != nullptr; np = np->Prev)
         {
...
double SparseMatrix::GetRowNorml1(int irow) const
{
   ...
   if (A)
      ...
      for (RowNode *np = Rows[irow]; np != nullptr; np = np->Prev)
      {
         a += fabs(np->Value);
      }
...
void SparseMatrix::Finalize(int skip_zeros, bool fix_empty_rows)
{
   ...
   {
      nr = 0;
      for (aux = Rows[i-1]; aux != nullptr; aux = aux->Prev)
   ...
   for (j = i = 0; i < height; i++)
   {
      int lastCol = -1;
      nr = 0;
      for (aux = Rows[i]; aux != nullptr; aux = aux->Prev)
      {
         if (!skip_zeros || aux->Value != 0.0)
         {
            J[j] = aux->Column;
...
void SparseMatrix::GetBlocks(Array2D<SparseMatrix *> &blocks) const
{
   ...
   for (int j = 0; j < bc; j++)
   {
      for (int i = 0; i < br; i++)
      {
         ...
         blocks(i,j) = new SparseMatrix(bI, nullptr, nullptr, nr, nc);
      }
   }
   ...
   for (int gr = 0; gr < height; gr++)
   {
      ...
      {
         for (RowNode *n_p = Rows[gr]; n_p != nullptr; n_p = n_p->Prev)
         {
            if (n_p->Value != 0.0)
            {
...
{
   if (A != nullptr)  // matrix is finalized
   {
      return I[height];
   }
   else
...
double SparseMatrix::MaxNorm() const
{
   ...
   {
      for (int i = 0; i < height; i++)
         for (RowNode *n_p = Rows[i]; n_p != nullptr; n_p = n_p->Prev)
         {
...
int SparseMatrix::CountSmallElems(double tol) const
{
   ...
   else
   {
      for (int i = 0; i < height; i++)
      {
         for (RowNode *aux = Rows[i]; aux != nullptr; aux = aux->Prev)
         {
...
void SparseMatrix::EliminateRow(int row, const double sol, Vector &rhs)
{
   ...
   for (aux = Rows[row]; aux != nullptr; aux = aux->Prev)
   {
                                   ...
                                   int d)
{
   ...
   {
      for (RowNode *aux = Rows[rc]; aux != nullptr; aux = aux->Prev)
      {
...
void SparseMatrix::EliminateRowCol(int rc, int d)
{
   ...
   else
   {
      ...
      for (aux = Rows[rc]; aux != nullptr; aux = aux->Prev)
      {
         if ((col = aux->Column) == rc)
         {
            if (d == 0)
...
void SparseMatrix::EliminateRowCol(int rc, SparseMatrix &Ae, int d)
{
   ...
   {
      RowNode *nd, *nd2;
      for (nd = Rows[rc]; nd != nullptr; nd = nd->Prev)
      {
         if ((col = nd->Column) == rc)
         {
            if (d == 0)
...
void SparseMatrix::Gauss_Seidel_forw(const Vector &x, Vector &y) const
{
   ...
   if (A == nullptr)
   {
      ...
      for (i = 0; i < s; i++)
      {
         sum = 0.0;
         diag_p = nullptr;
         for (n_p = R[i]; n_p != nullptr; n_p = n_p->Prev)
            if ((c = n_p->Column) == i)
            {
               diag_p = n_p;
            }
         ...
         if (diag_p != nullptr && diag_p->Value != 0.0)
         {
...
SparseMatrix &SparseMatrix::operator+=(const SparseMatrix &B)
{
   ...
   for (int i = 0; i < height; i++)
   {
      ...
      if (B.Rows)
      {
         for (RowNode *aux = B.Rows[i]; aux != nullptr; aux = aux->Prev)
         {
...
SparseMatrix &SparseMatrix::operator=(double a)
{
   if (Rows == nullptr)
      ...
      for (int i = 0; i < height; i++)
         for (RowNode *node_p = Rows[i]; node_p != nullptr;
              node_p = node_p -> Prev)
         {
            node_p -> Value = a;
         }
...
void SparseMatrix::Print(std::ostream & out, int _width) const
{
   ...
   if (A == nullptr)
   {
      ...
      {
         out << "[row " << i << "]\n";
         for (nd = Rows[i], j = 0; nd != nullptr; nd = nd->Prev, j++)
         {
...
{
   if (I != nullptr && ownGraph)
   {
      delete [] I;
   }
   if (J != nullptr && ownGraph)
   {
      delete [] J;
   }
   if (A != nullptr && ownData)
   {
      delete [] A;
   }
...
SparseMatrix Transpose(const SparseMatrix &A)
{
   ...
   int m = A.Height(); // number of rows of A
   int n = A.Width();  // number of columns of A
   ...
   std::vector<int> At_i(n+1);
   std::vector<int> At_j(nnz);
   std::vector<double> At_data(nnz);
   ...
   return SparseMatrix(std::move(At_i),
					   std::move(At_j),
					   std::move(At_data),
					   n, m);
}
...
SparseMatrix Mult(const SparseMatrix &A, const SparseMatrix &B)
{
   ...
   std::vector<int> C_i(nrowsA + 1);
   C_i[0] = 0;

   int num_nonzeros = 0;


   for (int ic = 0; ic < nrowsA; ic++)
   {
   ...
   std::vector<int> C_j(num_nonzeros);
   std::vector<double> C_data(num_nonzeros);

   std::fill(begin(B_marker), end(B_marker), -1);

   int counter = 0;
   for (int ic = 0; ic < nrowsA; ic++)
   {
      // row_start = C_i[ic];
      const int row_start = counter;

      for (int ia = A_i[ic]; ia < A_i[ic+1]; ia++)
      {
         const int ja = A_j[ia];
         const double a_entry = A_data[ia];

         for (int ib = B_i[ja]; ib < B_i[ja+1]; ib++)
         {
            const int jb = B_j[ib];
            const double b_entry = B_data[ib];
...
{
   const int nrowsA = A.Height();
   const int ncolsA = A.Width();

   const int nrowsB = B.Height();
   ...
   for (int ic = 0; ic < nrowsA; ic++)
   {
      ...
      for (int ia = 0; ia < colsA.Size(); ia++)
      {
         const int ja = colsA[ia];
         B.GetRow(ja, colsB, dataB);

         for (int ib = 0; ib < colsB.Size(); ib++)
         {
            const int jb = colsB[ib];
   ...
   for (int ic = 0; ic < nrowsA; ic++)
   {
      ...
      {
         const int ja = colsA[ia];
         const double a_entry = dataA[ia];
         B.GetRow(ja, colsB, dataB);

         for (int ib = 0; ib < colsB.Size(); ib++)
         {
            const int jb = colsB[ib];
            const double b_entry = dataB[ib];
            if (B_marker[jb] < row_start)
            {
               B_marker[jb] = counter;
               C_j[counter] = jb;
                   ...
                   const SparseMatrix & B)
{
   ...
   delete[] marker;
   return SparseMatrix(C_i, C_j, C_data, nrows, ncols);
}
...
SparseMatrix Add(Array<SparseMatrix>& Ai)
{
   ...
   {
      accumulate = Add(accumulate, Ai[i]);
   }
