...
DenseMatrix::DenseMatrix(const DenseMatrix& other)
    : Matrix(other.height, other.width), data(other.data), capacity(other.capacity)
{

}

DenseMatrix::DenseMatrix(int s) : DenseMatrix(s, s)
{

}

DenseMatrix::DenseMatrix(int m, int n) : Matrix(m, n),
    data(m * n, 0), capacity(m * n)
{
    MFEM_ASSERT(m >= 0 && n >= 0,
...
void DenseMatrix::Mult(const double* x, double* y) const
{
    ...
    for (int col = 1; col < width; col++)
    {
        ...
        for (int row = 0; row < height; row++)
        {
            y[row] += x_col * data[row + col * height];
        }
    }
}
...
void DenseMatrix::MultTranspose(const double* x, double* y) const
{
    for (int col = 0; col < width; col++)
    {
        ...
        for (int row = 0; row < height; row++)
        {
            y_col += x[row] * data[row + col * height];
        }
...
{
    std::fill(begin(data), end(data), c);
...
extern "C" 
{
    ...
    void dgetrs_(char*, int*, int*, const double*, int*,
...
void dsyev_Eigensystem(const DenseMatrix& a, Vector& ev, DenseMatrix* evect)
{
    ...
    dsyev_(&jobz, &uplo, &n, nullptr, &lda, nullptr,
           &qwork, &lwork, &info);
    ...
    // Compute Eigenproblem
    dsyev_(&jobz, &uplo, &n, a_copy.GetData(), &lda, w,
...
inline void vec_normalize3(const double& x1, const double& x2,
                           const double& x3,
                           double& n1, double& n2, double& n3)
{
...
double DenseMatrix::CalcSingularvalue(const int i) const
{
    ...
    else if (n == 2)
    {
        ...
        s = sqrt(0.5 * (d0 * d0 + d1 * d1 + d2 * d2 + d3 * d3) + sqrt(
                     t * t + s * s));
    ...
    else
    {
        ...
        {
            double b11_b22 = ((d0 - d3) * (d0 + d3) + (d1 - d4) * (d1 + d4) + (d2 - d5) *
                              (d2 + d5));
            double b22_b33 = ((d3 - d6) * (d3 + d6) + (d4 - d7) * (d4 + d7) + (d5 - d8) *
                              (d5 + d8));
            double b33_b11 = ((d6 - d0) * (d6 + d0) + (d7 - d1) * (d7 + d1) + (d8 - d2) *
                              (d8 + d2));
        ...
        R = (c1 * (b23 * b23 - c2 * c3) + b12 * (b12 * c3 - 2 * b13 * b23) + b13 * b13 *
             c2) / 2;
...
void DenseMatrix::CalcEigenvalues(double* lambda, double* vec) const
{
    ...
    else
    {
        ...
        R = (c1 * (d23 * d23 - c2 * c3) + d12 * (d12 * c3 - 2 * d13 * d23) + d13 * d13 *
             c2) / 2;
...
void DenseMatrix::Diag(double c, int n)
{
    ...
    std::fill(begin(data), end(data), 0.0);
...
void DenseMatrix::GradToDiv(Vector& div)
{
    ...
    std::copy(&div[0], &div[0] + n, begin(data));
}
...
void DenseMatrix::CopyMN(const DenseMatrix& A, int row_offset, int col_offset)
{
    for (int j = 0; j < A.Width(); j++)
    {
        ...
        {
            (*this)(row_offset + i, col_offset + j) = A.data[i + j * A.Height()];
        }
    }
}
                             ...
                             int col_offset)
{
    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++)
        {
            (*this)(row_offset + i, col_offset + j) = 0.0;
            (*this)(row_offset + j, col_offset + i) = 0.0;
        }
    }
...
void DenseMatrix::AddMatrix(DenseMatrix& A, int row, int col)
{
    const int h  = Height();
    ...
    for (int j = 0; j < aw; j++)
    {
        for (int i = 0; i < ah; i++)
        {
            data[(i + row) + (j + col) * h] += A.data[i + j * ah];
        }
    }
}

void DenseMatrix::AddMatrix(double a, DenseMatrix& A, int row, int col)
{
    const int h  = Height();
    const int ah = A.Height();
    const int aw = A.Width();
    ...
    for (int j = 0; j < aw; j++)
    {
        for (int i = 0; i < ah; i++)
        {
            data[(i + row) + (j + col) * h] +=  a * A.data[i + j * ah];
        }
    }
}
...
void DenseMatrix::GetFromVector(int offset, const Vector& v)
{
    ...
    std::copy(v + offset, v + offset + n, begin(data));
}
...
void LUFactors::Factor(int m)
{
    ...
    if (m > 0)
    {
        dgetrf_(&m, &m, data.data(), &m, ipiv.data(), &info);
    }
...
void LUFactors::Mult(DenseMatrix& X) const
{
    ...
    for (int k = 0; k < n; k++)
    {
        ...
        for (int i = m - 1; i >= 0; i--)
        {
            std::swap(x[i], x[ipiv[i] - ipiv_base]);
        }
    }
}
...
{
    const double* data = this->data.data();
    const int* ipiv = this->ipiv.data();
    double* x = X;
...
void LUFactors::Solve(int m, int n, double* X) const
{
    ...
    if (m > 0 && n > 0) { dgetrs_(&trans, &m, &n, data.data(), &m, ipiv.data(), X, &m, &info); }
...
void DenseMatrixInverse::Factor(const DenseMatrix& mat)
{
    ...
    const double* mat_data = mat.GetData();
    double* lu_data = lu.GetData();

    std::copy(mat_data, mat_data + width * height, lu_data);
...
void DenseMatrixInverse::SetOperator(const Operator& op)
{
    ...
    MFEM_VERIFY(p != nullptr, "Operator is not a DenseMatrix!");
    Factor(*p);
}
...
void DenseMatrixInverse::TestInversion(const DenseMatrix& a)
{
    DenseMatrix C(width);
    Mult(a, C);
...
DenseMatrixSVD::DenseMatrixSVD(DenseMatrix& d)
    : m(d.Height()), n(d.Width())
{
    Init();
}
...
DenseMatrix DenseMatrixSVD::EvalCopy(const DenseMatrix& M)
{
    DenseMatrix svd(M);
    Eval(svd);
...
#ifdef MFEM_USE_LAPACK
    dgesvd_(&jobu, &jobvt, &m, &n, M.Data(), &m, sv.GetData(), nullptr, &m,
            nullptr, &n, work.data(), &lwork, &info);
...
const
{
    ...
    if (n <= 4)
    {
        for (int i = 0; i < ne; ++i)
        {
            const int* dofs = J + I[i];

            for (int col = 0; col < n; col++)
            {
                double x_col = x(dofs[col]);

                for (int row = 0; row < n; row++)
                {
                    y(dofs[row]) += x_col * tdata[i](row, col);
                }
            }
        }
    }
    else
    {
        ...
        {
            const int* dofs = J + I[i];
            double x_col = x(dofs[0]);

            for (int row = 0; row < n; row++)
            {
                ye(row) = x_col * tdata[i](row, 0);
            }
